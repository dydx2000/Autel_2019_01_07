...
des_cap['platformVersion'] = '7' #lgG 7
...
def SetPictureMode(mode,num=None):
    ...
    try:
            driver.find_element_by_android_uiautomator('new UiSelector().text(mode)').click()  #先看有没有在显示,
            ...
            while i<=len(listMode) :   #通过参数指定模式的参数编号 seq 与 当前滑块 最左边的 所见元素的编号比较, seq比较大,就左移,否则右移
                ...
                if driver.find_element_by_android_uiautomator('new UiSelector().text(listMode[i])'):
                      flag = False
                      if i>seq:
                          j=0
...
#在主界面判断是否外在拍照状态
def captureStatusConfirm():
    if driver.find_element_by_id ("拍照按钮"):
        pass

    elif driver.find_element_by_id ("录相按钮"):
        driver.find_element_by_id ("照片录相切换按钮 ").click()

    elif driver.find_element_by_id ("指南针按钮 "):
        driver.tap ("小图中央")
...
#从任意画面切换到拍照页面
def SwitchtoCaptureMode():




    try:
        if  driver.find_element_by_id("dengluanniu"):  #在开始界面
            driver.find_element_by_id("开始按钮").click()
            if driver.find_element_by_id("拍照按钮"):
                pass
            else:
                driver.find_element_by_id("照片录相切换按钮 ").click()

        elif  driver.find_element_by_id("Intelligent Flight"): #在智能飞行界面
            driver.find_element_by_id("camera").click()
            captureStatusConfirm()

        elif  driver.find_element_by_id("settings"):     #在设置界面
            driver.find_element_by_id("关闭按钮").click()
            captureStatusConfirm()


        elif  driver.find_element_by_id("Camera Settings"): #在相机设置界面
            driver.tap("画面左中点一下")

            captureStatusConfirm()

        elif driver.find_element_by_id("Aircraft Status"): #在飞机状态画面
            driver.tap("画面中央点一下")
...
#滚动到 bottom Bar 所需要的设置位置:
def scrollToFunc(func):
    if func=='SIZE':
        seq=0
    elif func=='FORMAT':
        seq=1
        ...
        seq=7
    elif func=='DIGITAL ZOOM':
        seq=7


    listAll=['SIZE','FORMAT', 'MODE','EXPOSURE MODE','ISO','SHUTTER','COLOR','STYLE','DIGITAL ZOOM']

    try:
            driver.find_element_by_android_uiautomator('new UiSelector().text(func)').click()
    except:
        i = 0
        while i <= len (listAll):  # 通过参数指定模式的参数编号 seq 与 当前滑块 最左边的 所见元素的编号比较, seq比较大,就左移,否则右移

            if driver.find_element_by_android_uiautomator ('new UiSelector().text(listAll[i])'):
                flag = False
                if i > seq:
                    j = 0
